...
<?if($arResult["ITEMS"]):?>
    <div class="inline-flex w-full flex-col gap-5 md:gap-c_af">
        <div class="inline-flex items-baseline justify-between px-c_narrow lg:px-0">
            <div class="h1 dark:text-white"><?$APPLICATION->ShowTitle(false);?></div>
        </div>
...
<?else:?>
    <div class="inline-flex flex-col gap-2 px-c_narrow *:text-sm md:my-5 md:gap-c_md md:px-5 *:md:text-lg lg:px-0">
        <div class="dark:text-white">По вашим параметрам ничего не нашлось. Попробуйте сбросить фильтры.</div>
        <a href="<?=$arResult["DEF_URL"]?>" class="inline-flex items-center gap-c_xs text-warning">
