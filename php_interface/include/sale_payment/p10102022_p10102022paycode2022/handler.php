...
class p10102022_p10102022paycode2022Handler extends PaySystem\ServiceHandler implements PaySystem\IRefund
{
    ...
    private const STATUS_3DS_CODE = 'wait_for_3ds';
    ...
    /**
     * Возвращает массив данных для формирования чека
     *
     * @param Payment $payment
     * @return array
     */
    ...
    /**
     * Формирует массив товаров из корзины для чека
     * @param Payment $payment
     * @return array
        ...
        foreach ($basketItems as $key => $BasketItem) {
            ...
            foreach ($BasketItem->getPropertyCollection() as $property) {
                if ($property->getField('CODE') === 'BARCODE') {
                    ...
                    if ($barcode) {
                        $name = "{$barcode['UF_SERIES']} {$barcode['UF_TICKET_NUM']} {$name}";
                    }
                }
            ...
            $lenDiff = 127 - mb_strlen($name) - mb_strlen($seatName);
            if ($lenDiff < 0) {
                $name = mb_substr($name, 0, $lenDiff);
    ...
    private function getPayselectionPayment(Payment $payment): ServiceResult
    {
        ...
        if ($sendResult->isSuccess()) {
            ...
            $logger->info(
                'payselection ' .
                __CLASS__ . '::' . __FUNCTION__
                . "\nответ: " . var_export($paymentData, true)
            );
        } else {
            $result->addErrors($sendResult->getErrors());
        }
    ...
    /**
     * Обработка входящего запроса от PaySelection
     * @param Payment $payment
    ...
    public function processRequest(Payment $payment, Request $request): ServiceResult
    {
        ...
        if (!isset($data['Event'])) {
            ...
            if (!$this->isSumCorrect($payment, $data['Amount'])) {
                ...
                $logger->error(
                    'payselection ' .
                    __CLASS__ . '::' . __FUNCTION__
                    . ' 403 Forbidden: check fail, different sum.'
                );
                die();
            }
        ...
        if (!$payselectionPaymentResult->isSuccess()) {
            ...
            $logger->error(
                'payselection ' .
                __CLASS__ . '::' . __FUNCTION__
                . ' error: ' . var_export($payselectionPaymentResult->getErrors(), true)
            );
        ...
        if (!empty($payselectionPaymentData['Code'])) {
            ...
            $logger->error(
                'payselection ' .
                __CLASS__ . '::' . __FUNCTION__
                . ' status error: ' . $payselectionPaymentData['Description']
            );
        ...
        if (
            $payselectionPaymentData['TransactionState'] !== self::STATUS_SUCCESSFUL_CODE
            && $payselectionPaymentData['TransactionState'] !== self::STATUS_PREAUTORIZED_CODE
            && $payselectionPaymentData['TransactionState'] !== self::STATUS_3DS_CODE
            ...
            $logger->error(
                'payselection ' .
                __CLASS__ . '::' . __FUNCTION__
                . ' transaction status error: ' . $payselectionPaymentData['TransactionState']
            );
        ...
        if ($payselectionPaymentData['TransactionState'] !== self::STATUS_3DS_CODE) {
            if ($this->isSumCorrect($payment, $data['Amount'])) {
                $fields['PS_STATUS'] = 'Y';
                ...
                $logger->info(
                    'payselection ' .
                    __CLASS__ . '::' . __FUNCTION__
                    . ' change status: ' . $this->getBusinessValue($payment, 'PS_CHANGE_STATUS_PAY')
                );
    ...
    public static function isMyResponse(Request $request, $paySystemId): bool
    {
        ...
        if ($inputStream) {
            ...
            if (isset($data['OrderId'])) {
                try {
                    $order = Order::loadByAccountNumber($data['OrderId']);
                ...
                foreach ($paymentCollection as $payment) {
                    ...
                    if ($paySystemName === 'p10102022_p10102022paycode2022') {
                        return true;
                    }
                }
            }
     ...
     * @return int|bool
     */
    public function getPaymentIdFromRequest(Request $request): int|bool
    {
        $inputStream = static::readFromStream();
        if ($inputStream) {
            $data = static::decode($inputStream);
            if (isset($data['OrderId']) && isset($data['ReceiptData']['external_id'])) {
                [$paymentId] = explode(self::TRACKING_ID_DELIMITER, $data['ReceiptData']['external_id']);

                try {
                    $order = Order::loadByAccountNumber($data['OrderId']);
                } catch (Main\ArgumentNullException|Main\ArgumentException|Main\NotImplementedException $e) {
                    return false;
                }
